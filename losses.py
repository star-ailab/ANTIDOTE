...
class antidote_alpha (nn.Module):
    def __init__(self, a_, b_, k_, alpha_, lambda_, rho_, delta_, c_, lr_):
        super(antidote_alpha, self).__init__()
        self.lambda_ = nn.Parameter(torch.tensor(lambda_, requires_grad=True))
        self.rho_ = torch.tensor(rho_)
        self.a_ = a_
        self.b_ = b_
        self.k_ = k_
        self.alpha_ = alpha_
    ...
    def forward(self, yp, y):
        minLoss = (-self.lambda_ * self.delta_) - self.rho_ -\
                  (self.lambda_+ self.k_) * ( torch.mean(f_star( - (loss_fct(yp,y)+self.rho_)/(self.lambda_ + self.k_), self.alpha_) )  ) 
        
        return minLoss
    
class antidote_kl (nn.Module):
    def __init__(self, a_, b_, k_, lambda_, delta_, c_, lr_):
        super(antidote_kl, self).__init__()
        self.lambda_ = nn.Parameter(torch.tensor(lambda_, requires_grad=True))
        self.a_ = a_
        self.b_ = b_
        self.k_ = k_
    ...
    def forward(self, yp, y):
        loss_ = -(loss_fct(yp,y))/(self.lambda_ + self.k_)
        minLoss = (-self.lambda_ * self.delta_) - \
                  (self.lambda_+ self.k_) * ( torch.log(torch.mean(torch.exp(loss_ - loss_.max()) ) ) + loss_.max() ) 
        
        return minLoss
    
class antidote_kl_bisec (nn.Module):
    def __init__(self, k_, lambda_, delta_, c_, lr_):
        super(antidote_kl_bisec, self).__init__()
