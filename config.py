...
def get_antidote_alpha(args):
    return antidote_alpha(a_=-5., b_=5., k_=args.k_, alpha_=args.alpha_, lambda_=args.lambda_, rho_ =args.rho_, delta_=args.delta_, c_=1., lr_=args.lr_)

def get_antidote_kl(args):
    return antidote_kl(a_=-5., b_=5., k_=args.k_, lambda_=args.lambda_, delta_=args.delta_, c_=1., lr_=args.lr_)

def get_antidote_kl_bisec(args):
    return antidote_kl_bisec(k_=args.k_, lambda_=args.lambda_, delta_=args.delta_, c_=1., lr_=args.lr_)

    
def get_loss_config(args):
    if args.loss == 'antidote_alpha':
        return get_antidote_alpha(args)
    if args.loss == 'antidote_kl':
        return get_antidote_kl(args)
    if args.loss == 'antidote_kl_bisec':
        return get_antidote_kl_bisec(args)
    if args.loss in ['ECEandMAE', 'EFLandMAE']:
